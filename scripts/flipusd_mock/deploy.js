...
function defaultConfigPath(root, networkName) {
  return path.join(root, "config", "flipusd_mock", `deployConfig-${networkName}.json`);
}
...
async function main() {
  ...
  console.log("Valid:", cfg.valid);

  // Deploy
  const Factory = await ethers.getContractFactory("MockPriceProvider");
  const priceProvider = await Factory.deploy(toRay(cfg.price), cfg.valid);
  const rcpt = await priceProvider.deploymentTransaction().wait();

  console.log("Mock Price Provider deployed at:", priceProvider.target);
