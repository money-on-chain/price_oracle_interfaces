...
function loadConfig(networkName) {
  ...
  const cfgPath =
    ...
    path.join(__dirname, `../../config/flipusd_mock/deployConfig-${networkName}.json`);
...
async function main() {
  ...
  const constructorArgs = [toRay(cfg.price), cfg.valid];
  ...
  console.log("=== Verify MockPriceProvider ===");
  ...
  console.log("  valid     :", constructorArgs[1]);
