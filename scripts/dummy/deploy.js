...
function defaultConfigPath(root, networkName) {
  return path.join(root, "config", "dummy", `deployConfig-${networkName}.json`);
}
...
async function main() {
  ...
  // Deploy
  const Factory = await ethers.getContractFactory("PriceProviderDummy");
  const priceProvider = await Factory.deploy(toRay(cfg.price));
  const rcpt = await priceProvider.deploymentTransaction().wait();

  console.log("Price Provider Dummy deployed at:", priceProvider.target);
