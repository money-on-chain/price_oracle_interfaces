...
function loadConfig(networkName) {
  ...
  // 2) <repoRoot>/config/flipbpro/deployConfig-<network>.json
  const cfgPath =
    process.env.DEPLOY_CONFIG_PATH ??
    path.join(__dirname, `../../config/flipbpro/deployConfig-${networkName}.json`);
...
async function main() {
  ...
  requireKeys(cfg, ["MoCState", "FlipUsd", "priceProviderAddress"]);
  ...
  const constructorArgs = [cfg.FlipUsd, cfg.MoCState];
  ...
  console.log("=== Verify PriceProviderFlipPerBpro ===");
  console.log("Network         :", net);
  console.log("Config          :", cfgPath);
  console.log("Address         :", address);
  console.log("Provider        :", provider);
  console.log("Constructor args:");
  console.log("  FlipUsd     :", constructorArgs[0]);
