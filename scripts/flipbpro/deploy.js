...
function defaultConfigPath(root, networkName) {
  return path.join(root, "config", "flipbpro", `deployConfig-${networkName}.json`);
}
...
async function main() {
  ...
  assertAddress("FlipUsd", cfg.FlipUsd);
  console.log("FlipUsd:", cfg.FlipUsd);

  // Deploy
  const Factory = await ethers.getContractFactory("PriceProviderFlipPerBpro");
  const priceProvider = await Factory.deploy(cfg.FlipUsd, cfg.MoCState);
  const rcpt = await priceProvider.deploymentTransaction().wait();

  console.log("Price Provider Flip/BPRO deployed at:", priceProvider.target);
