...
function defaultConfigPath(root, networkName) {
  return path.join(root, "config", "bprousdv1", `deployConfig-${networkName}.json`);
}
...
async function main() {
  ...
  // Deploy
  const Factory = await ethers.getContractFactory("PriceProviderBproUsdV1");
  const priceProvider = await Factory.deploy(cfg.MoCState);
  const rcpt = await priceProvider.deploymentTransaction().wait();

  console.log("Price Provider Bpro Usd V1 deployed at:", priceProvider.target);
