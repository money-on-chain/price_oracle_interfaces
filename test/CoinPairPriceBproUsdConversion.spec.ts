...
describe("CoinPairPriceBproUsdConversion", () => {
  it("reverts on zero addresses in constructor", async () => {
    ...
    await expect(Factory.deploy(ethers.ZeroAddress, moc.target)).to.be.revertedWith(
      "coinpairprice address is zero",
    );

    await expect(Factory.deploy(base.target, ethers.ZeroAddress)).to.be.revertedWith(
